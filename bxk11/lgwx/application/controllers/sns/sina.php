...
class Sina extends CI_Controller {
	...
	public function index(){
		...
		echo "<a href=".$code_url."><img src='".$_SERVER['HTTP_HOST']."/application/libraries/sns/sina/weibo_login.png' title='点击进入授权页面'' alt='点击进入授权页面'' border='0' /></a>";exit;
	}
	...
	public function authorize(){
		...
		if (isset($_REQUEST['code'])) {
			...
			try {
				...
				$this->goBackUrl($e->getMessage(),$this->current_url);
			}
		}
		...
		if (isset($token) && $token) {
			...
			if(!$this->isRegist()){
				...
				if(isset($user_message['error_code'])){
					...
					//授权失败
					$this->goBackUrl($user_message['error'],$this->current_url);
