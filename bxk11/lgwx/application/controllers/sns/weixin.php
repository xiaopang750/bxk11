...
class Weixin extends CI_Controller {
        ...
        if (isset($token) && $token) {
            ...
            if(!$cooperationInfo){
                ...
                if($tokens){
                    ...
                    $_SESSION['user_id'] = $cooperationInfo->user_id;
	...
	private function goBackUrl($msg='',$errorurl){
		unset($_SESSION['current_url']);
		if($this->current_url) jumpAjax($msg,$this->current_url); else jumpAjax($msg,$errorurl);
	}
	/**
	 *description: 登录注册控制
	 *author: liuguangping
		...
		if($this->userData){
			...
			if($unionid){
				...
				if($res){
					...
					$picdata['user_pic_b'] = $b_sourceUrl;
					$picdata['user_pic_m'] = $m_sourceUrl;
					$picdata['user_pic_s'] = $s_sourceUrl;
					$where['user_id'] = $res->user_id;
					$this->userMode->updates_global($picdata,$where);
			...
			if($insert_id = $this->userMode->insert()){
				...
				if($this->userMode->updates_global($data,array('user_id'=>$insert_id))){
					...
					if(mkdirs($avatar_dir)){
						...
						$this->saveImage($b_soruceUrl,$avatar_dir_b);
						$this->saveImage($m_soruceUrl,$avatar_dir_m);
						$this->saveImage($s_soruceUrl,$avatar_dir_s);
						*/
	 ...
	 *date:2013/12/27
	 */
	 private function saveImage($source_url,$dest_url){
	     ob_start();
	     readfile($source_url);
	     $img_data = ob_get_contents();
	     ob_end_clean();
	     $local_file = fopen($dest_url , 'a');
