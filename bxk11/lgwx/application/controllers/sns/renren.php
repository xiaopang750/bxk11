...
class Renren extends CI_Controller {
	...
	public function index(){
		...
		echo "<a href=".$code_url."><img src='".$_SERVER['HTTP_HOST']."/application/libraries/sns/renren/renren.png' title='点击进入授权页面'' alt='点击进入授权页面'' border='0' /></a>";exit;
	...
	public function authorize(){
		...
		if (isset ( $_REQUEST ['code'] )) {
			...
			if (!isset($_SESSION ['renren_state']) || empty ( $state ) || $state !== $_SESSION ['renren_state']) {
				$this->goBackUrl('非法请求！',$this->current_url);exit;
			}
			...
			try {
				...
				$error =  $e->getMessage();
				$this->goBackUrl($error,$this->current_url);exit;
		...
		if (isset($token) && $token) {
			...
			if($insert_id = $this->renrenConsole()){
				...
				$this->goBackUrl('授权失败。',$this->current_url);exit;
