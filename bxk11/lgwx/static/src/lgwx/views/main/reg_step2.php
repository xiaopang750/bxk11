...
<script type="text/html" id="tpl-step2">

	<div class="form-group">
		<label class="label-control col-1 fl">从已添加列表中选择参与认证的品牌:</label>
		<select class="fl form-control col-3" name="province_code" form_check="sys" sc="select">
			<option id="" value="">请选择</option>
			{{each data.brand_list}}
			<option id="{{$value.apply_id}}">{{$value.apply_brand_name}}</option>
			{{/each}}
		</select>
	</div>
	...
	<div class="form-group" script-role="check_wrap">
		...
		<div class="col-3 fl">
			<input type="text" class="form-control" name="apply_brand_name" form_check="sys" ischeck="true" tip="此项为必填" wrong="请填写10位以内的中文名称" re="(.{1,10})" value="{{data.apply_brand_name}}" />
	...
	<div class="form-group" script-role="check_wrap">
		...
		<div class="col-3 fl">
			<input type="text" class="form-control" name="apply_brand_ename" form_check="sys" ischeck="false" tip="此项为必填" wrong="请填写20位以内的英文名称" re="(.{1,20})" value="{{data.apply_brand_ename}}" />
	...
	<div class="form-group" script-role="check_wrap">
		...
		<div script-role="check_wrap">
			...
			<div class="col-1 fl">
				<input type="text" sc="time-select" class="form-control" name="apply_license_begin" form_check="sys" ischeck="false" tip="此项为必填" wrong="日期格式不正确" re="([0-9]{4}\-[0-9]{2}\-[0-9]{2})" value="{{data.apply_license_begin}}" />
		...
		<div script-role="check_wrap">
			...
			<input type="text" sc="time-select" class="form-control col-1 fl" name="apply_license_end" form_check="sys" ischeck="false" tip="此项为必填" wrong="日期格式不正确" re="([0-9]{4}\-[0-9]{2}\-[0-9]{2})" value="{{data.apply_license_end}}">
		</div>
		...
		<form action="/lgwx/index.php/upload/serviceApplyBrand" enctype="multipart/form-data" target="uploadframe" script-role="upload-form" method="post">
			<div script-role="check_wrap">
				<label class="label-control fl col-2">
					...
					<p class="gray">
						请上传加盖公章的商标注册或代理经销授权书的清晰彩色原件扫描件，支持jpg,gif,bmp格式照片，大小不超过512k
					</p>
				</label>
	...
	<div class="form-group">
		...
		<div script-role="check_wrap">
			<label class="label-control fl col-1">
				...
				<p class="gray">
					上传大小不得超过于512k的jpg,gif,png,bmp格式图片
				</p>
			</label>
	...
	<div class="form-group">
		...
		<div class="col-5 fl mt_6">
			<a class="btn btn-success small" sc="back" href="javascript:;"><span class="mr_10">《上一步</span></a>
			<a class="btn btn-danger small" script-role="confirm_btn" href="javascript:;"><span class="mr_10">下一步</span>》</a>
		</div>
	</div>
</script>
