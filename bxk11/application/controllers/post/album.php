...
class Album extends User_Controller {
	...
	public function add(){
		...
		$data['config'] = $config;
		if($res){
			jumpAjax('添加成功！',U($config['list']));
		}else{
			jumpAjax('添加失败！',U($config['list']));
		}
	...
	public function edit(){
		...
		$where = array('album_id'=>$id,'user_id'=>$user_id);
		...
		$data['config'] = $config;
		if($res){
			jumpAjax('编辑成功！',U($config['list']));
		}else{
			jumpAjax('编辑失败！',U($config['list']));
		}
	}
	...
	public function del(){
		...
		$res = $this->t_album_model->delete($id);
		...
		if($res){
			jumpAjax('删除成功！',U($config['list']));
		}else{
			jumpAjax('删除失败！',U($config['list']));
		}
	}
