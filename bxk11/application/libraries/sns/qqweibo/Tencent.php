...
*/
class JiaException extends Exception {
    // pass
}
...
class OAuth
{
     ...
     * 验证授权是否有效
     */
    public static function checkOAuthValid($param)
    {   
        //抛出异常 直接返出给 try catch(Exception)
        if(!$param) throw new JiaException("请传入参数！");
...
class Tencent
{
    ...
    {   
        ...
        } elseif (isset(self::$params['openid']) && isset(self::$params['openkey'])) {//openid & openkey方式
            $params['appid'] = OAuth::$client_id;
            $params['openid'] = self::$params['openid'];
            $params['openkey'] = self::$params['openkey'];
            $params['clientip'] = Common::getClientIp();
            $params['reqtime'] = time();
            $params['wbversion'] = '1';
