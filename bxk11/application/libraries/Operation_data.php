...
class Operation_data {
	 ...
	 * @param string $pat_arr
	 */
	public function downinformation_file($pat_arr=''){
		...
		$objProps->setLastModifiedBy("jia178");
		//标题
		$objProps->setTitle("jia178_information");
		//题目
		$objProps->setSubject("jia178_information");
		//描述
		$objProps->setDescription("Test document, jia178 by PHPExcel.");
		//关键字
		$objProps->setKeywords("jia178_information");
		//类别
		$objProps->setCategory("jia178_information");
		//设置当前的sheet
		$objExcel->setActiveSheetIndex(0);
		$objActSheet = $objExcel->getActiveSheet();
		//设置当前活动sheet的名称
		$objActSheet->setTitle('jia178_information');
		//*************************************
		//设置单元格内容
		//由PHPExcel根据传入内容自动判断单元格内容类型
		$objActSheet->setCellValue('A1', "系统资讯");
		$objActSheet->setCellValue('A2', "资讯分类");
		$objActSheet->setCellValue('B2', "资讯标题");
		$objActSheet->setCellValue('C2', "资讯封面图（630*420）");
		$objActSheet->setCellValue('D2', "资讯摘要");
		$objActSheet->setCellValue('E2', "资讯正文");
		$objActSheet->setCellValue('F2', "资讯发布日期");
		$objActSheet->setCellValue('G2', "资讯关键词");
		...
		$objAlignA1->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		$objAlignA1->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$objActSheet->getStyle('A2:G2')->getFont()->setSize(12)->setBold(true);
	
		//垂直居中
		$objActSheet->getStyle('A1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
		$outputFileName = "JIA178_INFORMATION.xls";
		header("Content-Type:application/octet-stream;charset=utf-8");
		header('Content-Disposition: attachment; filename=' . $outputFileName);
