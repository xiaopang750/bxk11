								...
								<div class="row-fluid search-forms search-default">
									<form class="form-search" action="<?php echo site_url('admin/member/index')?>" method='get'>
										...
										<select class="header-option m-wrap small" name='service_status'>
											...
											<option value="1" <?php if($service_status == 1) echo "selected";?>>普通</option>
											<option value="11" <?php if($service_status == 11) echo "selected";?>>vip</option>
											<option value="12" <?php if($service_status == 12) echo "selected";?>>停用</option>
											<option value="21" <?php if($service_status == 21) echo "selected";?>>vip已到期</option>
											<option value="12" <?php if($service_status == 12) echo "selected";?>>审核中</option>
								...
								<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
									...
									<tbody>
										...
										<tr id="t_s<?php echo $val->service_id;?>">
											...
											<td><?php if(!$val->service_ename || empty($val->service_ename)) echo "暂无";else echo  cn_substr_utf8($val->service_ename,0,10,false);?></td>
											
										<!-- 	<td><?php if(!$val->service_name || empty($val->service_name)) echo "暂无";else echo  cn_substr_utf8($val->service_name,0,10,false);?></td> -->
								
											<td><?php if(!$val->service_company || empty($val->service_company)) echo "暂无";else echo  cn_substr_utf8($val->service_company,0,10,false);?></td>

											<td><?php if(empty($val->service_person) || is_null($val->service_person)) echo "暂无";else echo  cn_substr_utf8($val->service_person,0,20,false);?></td>
											...
											<ul class="nav">
												<li class="dropdown user">
													...
													<ul class="dropdown-menu">
														...
														<?php if($val->service_status != 1){?>
														<li><a href="#" onclick="jsv.status('1',<?php echo $val->service_id; ?>,'service');"><span class="label label-success">普通</span></a></li>
														<?php }?>
														<?php if($val->service_status != 2){?>
														<li><a href="#" onclick="jsv.status('2',<?php echo $val->service_id;?>,'service');"><span class="label label-danger">vip</span></a></li>
														<?php }?>
														<?php if($val->service_status != 3){?>
														<li><a href="#" onclick="jsv.status('3',<?php echo $val->service_id;?>,'service');"><span class="label label-info">vip已到期</span></a></li>
														<?php }?>
														
														<?php if($val->service_status != 11){?>
														<li><a href="#" onclick="jsv.status('11',<?php echo $val->service_id;?>,'service');"><span class="label-warning">审核中</span></a></li>
														<?php }?>
														
														<?php if($val->service_status != 12){?>
														<li><a href="#" onclick="jsv.status('12',<?php echo $val->service_id;?>,'service');"><span class="label-warning">停用</span></a></li>
														<?php }?>

														<?php if($val->service_status != 81){?>
														<li><a href="#" onclick="jsv.status('81',<?php echo $val->service_id;?>,'service');"><span class="label-warning">屏蔽</span></a></li>
														<?php }?>
														
														<?php if($val->service_status != 99){?>
														<li><a href="#" onclick="jsv.status('99',<?php echo $val->service_id;?>,'service');"><span class="label-warning">删除</span></a></li>
														<?php }?>
													</ul>
											...
											<td><a class="edit" href="javascript:jsv.go('<?php echo site_url('admin/member/edit').'?service_id='.$val->service_id;?>');">Edit</a><!-- |<a href="#" onclick="jsv.updatePwd(<?php echo $val->service_id;?>,'service')">密码重置</a> --></td>
										</tr>
										<?php }?>
									</tbody>
									<tfooter>
