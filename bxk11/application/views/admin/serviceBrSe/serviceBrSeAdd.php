					...
					<div class="span12">
						...
						<div class="portlet box blue">
							...
							<div class="portlet-body">
								<form action="<?php echo site_url('admin/serviceBrSe/doServiceBrSeAdd');?>" enctype="multipart/form-data" method="POST" 	onsubmit='return jsv.checkbrands_series_add();'>
								<table class="table table-hover">
									...
									<tbody>
										...
										<tr>
											...
											<td colspan='4'>
												<select class="header-option m-wrap small" name='service_id' id="service_id" onchange="jsv.serviceBrSe();">
													<?php if($service){ foreach($service as $ky=>$vaue){?>
														<option value="<?php echo $vaue->service_id;?>"  <?php if(isset($service_id) && $service_id && $service_id == $vaue->service_id) echo "selected";?> ><?php echo $vaue->service_company;?></option>
													<?php }}?>
												</select>
											</td>
											...
											<td colspan='4'>
												<select class="header-option m-wrap small" name='s_c_tag_id' id="s_c_tag_id" onchange="jsv.changeSeriesC()">
													...
													<?php if($brands){ foreach($brands as $key=>$value){?>
														<option value="<?php echo $value['brand_id'];?>" <?php if(isset($brand_id) && $brand_id && $brand_id == $value['brand_id']) echo "selected";?>><?php echo $value['apply_brand_name'];?></option>
													<?php }}?>
												</select>
											</td>
										...
										<tr>
												...
												<td colspan='4'>
													<select class="header-option m-wrap medium" name='series_status' id='series_status'>
														...
														<option value="2" selected>经销商私有</option>
										...
										<tr>
											<input type='hidden'  value="<?php if(isset($service_id) && $service_id) echo "$service_id";?>"  name='service_id_bak'/>
											<input type='hidden'  value="<?php if(isset($brand_id) && $brand_id) echo "$brand_id";?>"  name='brand_id_bak'/>
