...
class UsersAction extends BaseAction{
	...
	public function checkreg(){
		...
		if($db->create()){
			$id=$db->add();
			if($id){
				$data['service_name'] = $this->_post('username','trim');
				$data['service_passwd'] = md5($this->_post('repassword','trim'));
				$data['service_status'] = 1;
				$model = M();
				$model->query("INSERT INTO jia178.t_service_info (service_name,service_passwd,service_status) VALUES ('".$data['service_name']."','".$data['service_passwd']."',".$data['service_status'].")");
