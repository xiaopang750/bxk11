...
class Model {
    ...
    public function n_add($data='',$options=array(),$replace=false) {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            if(!empty($this->data)) {
                $data           =   $this->data;
	...
	/**
	 *description:sql查询，返回一条对象数据
	 *author:yanyalong
	 *date:2014/02/21
	 */
    public function row($sql,$parse=false) {
        if(!is_bool($parse) && !is_array($parse)) {
            $parse = func_get_args();
            array_shift($parse);
        }
	...
	/**
	 *description:sql查询，返回对象结果集
	 *author:yanyalong
	 *date:2014/02/21
	 */
    public function result($sql,$parse=false) {
        if(!is_bool($parse) && !is_array($parse)) {
            $parse = func_get_args();
            array_shift($parse);
        }
